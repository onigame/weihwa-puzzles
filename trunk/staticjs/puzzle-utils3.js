...
function _IG_GameGrid() {
  ...
  //   _IGGG_getPossibleTargetView(target, source)
  //     returns the HTML content of the cell when it is a possible target (but not the target) and the user is dragging.
  //   _IGGG_getDragView(index)
  //     returns the HTML content of the cell when it is neither the target nor the source and the user is dragging.
  ...
  this.dataObject._IGGG_refreshCell = function (index) {
    ...
    _gel(name).innerHTML = callback_this.getCellValue(index);
  }
    
  this.dataObject._IGGG_addDragSource = function (index) {
    if (index < 0) return; if (index > callback_this.maxIndex) return;
    var name = callback_this.getCellName(index);
    callback_this.dragHandler.addSource(name, _gel(name), callback_this.dataObject._IGGG_getSurrogateView(index));
  }
  ...
  this.dataObject._IGGG_refreshDragSources = function () {
    ...
    for (var i = 0; i <= callback_this.maxIndex; ++i) {
      if (callback_this.dataObject._IGGG_isDragSource(i)) {
        ...
        callback_this.dragHandler.addSource(name, _gel(name), callback_this.dataObject._IGGG_getSurrogateView(i));
      }
    }
  }
...
_IG_GameGrid.prototype.getCellValue = function (index) {
  if (!this.dragHandler.isDragging) {
    ...
    return this.dataObject._IGGG_getDragView(this.target, this.target);
  }
}

_IG_GameGrid.prototype.getTableHTML = function () {
  var result = "";
  result += '<table border="0" cellpadding="0px" cellspacing="0px">';
  for (var row = 0; row < this.height; ++row) {
    result += '<tr>';
    for (var col = 0; col < this.width; ++col) {
      result += '<td align="center" id="' + this.getCellTDID(row * this.width + col) + '">';
      result += '<span id="' + this.getCellName(row * this.width + col) + '">';
      result += this.getCellValue(row * this.width + col);
      result += '</span>';
      result += '</td>';
    }
    result += '</tr>';
...
//  yadda yadda 
  ...
  _IG_puzzle_pref_controller.prototype.setPrefs = function() {
    this.prefs.set(this.pref_name, this.game_state.toJSONString());
    if (this.navigation) {
      this.update_navbar();
    }
  }
