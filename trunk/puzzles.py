...
UserNotLoggedIn = "puzzle user not logged in"
InvalidUID = "the puzzle uid was invalid:"
...
class User(db.Model):
  ...
  uid = db.StringProperty()      # should be the same as the key.
...
# returns the UID of the user that is logged in
# input is the url of the page to go to after a log in if the user
# is not logged in
def CurrentUID():
  guser = users.get_current_user()
  if not guser:
    raise UserNotLoggedIn
  return GetOrCreateUser(guser.email()).key().name()

def CurrentUser():
  guser = users.get_current_user()
  if not guser:
    raise UserNotLoggedIn
  return GetOrCreateUser(guser.email())
...
class UserIdPage(webapp.RequestHandler):
  def get(self):
    u = users.get_current_user()
    if not u:
      self.redirect(users.create_login_url(self.request.url))
      return
    user = GetOrCreateUser(u.email())
    template_values = {
      'user': user,
    }
    path = os.path.join(os.path.dirname(__file__), 'user_id.html')
    self.response.out.write(template.render(path, template_values))
...
def UrlMappings():
  url_mappings = []
  url_mappings.append(('/user_id.html', UserIdPage))
  url_mappings.extend(duce.UrlMappings())
  return url_mappings
